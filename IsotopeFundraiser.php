...
class IsotopeFundraiser extends IsotopeProductCollection
{
	 ...
	 * @var string
	 */
	protected $strTable = 'tl_iso_fundraiser';

	/**
	 * Name of the child table
	 * @var string
	 */
	protected $ctable = 'tl_iso_fundraiser_items';
	...
	/**
	 * Return Fundraiser data. All data is cached for speed improvement.
	 *
	 * @access public
	 * @param string $strKey
	 * @return mixed
	...
	/**
	 * Load current user's fundraiser
	 */
	public function initializeFundraiser($arrData=array())
	{
		...
		if (!$this->blnRecordExists)
		{
			...
			{
				throw new Exception('Unable to create fundraiser');
			}
		}
	}
	...
	/**
	 * Update the products sold count for a given fundraiser
	 */
	public function updateSold()
	{
		$objItems = $this->Database->prepare("SELECT pid, product_id, SUM(product_quantity) as quantity, (SELECT status FROM tl_iso_orders WHERE tl_iso_orders.id=tl_iso_order_items.pid) as status FROM tl_iso_order_items WHERE fundraiser_id=? GROUP BY product_id")->execute($this->id);
	...
	/**
	 * Return array of info about a specific product in the fundraiser (quantity, options, etc)
	 */
	public function getProductInfo($objProduct)
	{
		$arrInfo = array();
		$arrInfo = $this->Database->prepare("SELECT * FROM tl_iso_fundraiser_items WHERE pid={$this->id} and product_id=?")->limit(1)->execute($objProduct->id)->fetchAllAssoc();
	...
	/**
	 * Transfer product from Fundraiser to Cart
	 * Modified from parent class to take specific product, quantity parameter and fundraiser id instead of entire transfer
	 */
	public function transferToCart(IsotopeProductCollection $objCollection, $objProduct, $intQuantity, $blnDuplicate=true)
	{
		...
		{
			$objNewItem = $this->Database->execute("SELECT * FROM {$objCollection->ctable} WHERE pid={$objCollection->id} AND product_id={$objOldItem->product_id} AND product_options='{$objOldItem->product_options}' AND fundraiser_id='{$this->id}'");
			...
			{
				$arrSet = array('pid'=>$objCollection->id, 'tstamp'=>$time, 'product_quantity'=>$intQuantity, 'fundraiser_id'=>$this->id);

				foreach( $objOldItem->row() as $k=>$v )
				{
					if (in_array($k, array('id', 'pid', 'tstamp', 'product_quantity', 'fundraiser_id')))
						continue;
	...
	/**
	 * Fetch products from database. Modified from parent to convert product objects to FundraiserProducts so we don't need to validate options
	 *
	 * @access public
	 * @return array
	 */
	public function getProducts($strTemplate='', $blnNoCache=false)
	{
		if (!is_array($this->arrProducts) || $blnNoCache)
		{
			...
			while( $objItems->next() )
			{
				...
				try
				{
					$objProduct = new FundraiserProduct($objProductData->row());
				}
				catch (Exception $e)
				{
					$objProduct = new FundraiserProduct(array('id'=>$objItems->product_id, 'sku'=>$objItems->product_sku, 'name'=>$objItems->product_name, 'price'=>$objItems->price));
				}
